...
func loadData(CardArray): # chamado por CARDDISPLAY
	...
	#		$NAME.get_font("normal_font").extra_spacing_char=proporcao
		...
		match DA[8]:
			"Monster":
				...
				$BG.texture_normal=load("res://BIBLIOTECA/CardSetup/Template/Monster"+String(DA[9])+"_Card.png")
				$ATTRIBUTE.visible=true # visivel pra monstros e skill
				$ATTRIBUTE.texture_normal=load("res://BIBLIOTECA/CardSetup/Attribute/"+DA[12]+".png")
				$LV.visible=true
				var i = 1
				while i<=12:
					if i<=int(DA[13]):
						get_node("LV/LV"+String(i)).visible=true
					else:
						get_node("LV/LV"+String(i)).visible=false
					i+=1
			...
			"Skill":
				$LV.visible=false #Só monstro tem LV
				$SET.set("custom_colors/default_color",Color(1,1,1))
				$NAME.get_font("normal_font").outline_size=3
				$BG.texture_normal=load("res://BIBLIOTECA/CardSetup/Template/Skill_Card.png")
	...
	#			$BG.texture_normal=load(DA[18])
			...
			_: #Spell / Trap
				$LV.visible=false #Só monstro tem LV
				$NAME.get_font("normal_font").outline_size=0 #sem contorno (só pra skill)
				$BG.texture_normal=load("res://BIBLIOTECA/CardSetup/Template/"+String(DA[8])+"_Card.png")
				$ATTRIBUTE.visible=true
				$ATTRIBUTE.texture_normal=load("res://BIBLIOTECA/CardSetup/Attribute/"+String(DA[8])+".png")
				...
				if CardArray[9]=="Normal":
					...
					$SPELLTRAP.texture_normal=load("res://BIBLIOTECA/CardSetup/Type/"+String(DA[9])+".png")
					$SPELLTRAP_TXT.bbcode_text="[right]["+String(CardArray[8]+" Card   ][/right]")
	#	var SP
	...
	#			$NAME.get_font("normal_font").outline_size=5
		...
		visibleArray=[$SecretRare.visible,$BG.visible,$ATTRIBUTE.visible,$LV.visible,$SPELLTRAP_TXT.visible,$PROPERTIES.visible,$DESC.visible,$ATK.visible,$DEF.visible,$SET.visible,$NAME.visible]
...
func loadIllustration(newID):
	...
	if(String(GLOBAL.storedPics[ID])!=IMG_URL and downloading==false): #Verifica se houve alteração do link
		...
		$Illustration.texture_normal=load("res://BIBLIOTECA/downloading.jpg") 
		var http_error = http_request.request(IMG_URL)
		if http_error != OK:
			print("An error occurred in the HTTP request.")
			downloading=false
			$Illustration.texture_normal=load("res://BIBLIOTECA/erro.jpg")
	elif(String(GLOBAL.storedPics[ID])!=IMG_URL and downloading==true):
		$Illustration.texture_normal=load("res://BIBLIOTECA/tryagain.jpg") 
		...
		if file.file_exists("user://cards_ID_"+String(ID)+".png"):
			...
			$Illustration.texture_normal=texture_normal
		elif (downloading==true):
			$Illustration.texture_normal=load("res://BIBLIOTECA/tryagain.jpg") 
		else:
			GLOBAL.storedPics[ID]="0"
			$Illustration.texture_normal=load("res://BIBLIOTECA/tryagain1.jpg") 
	else:
		print("loadIllustration resultou em ELSE!!!!!!")
		$Illustration.texture_normal=load("res://BIBLIOTECA/404.png") 
...
func grayscale(bool_key):
	...
	var i = 1;
	while i<=12:
		get_node("LV/LV"+String(i)).set_material(SP)
		i+=1
