...
func update_cards(newPool,new_pg): #AA #
	...
	if(new_pg<=0):
		$LeftArrow.visible=false
	if(newPool.size()-1<=40):
		$RightArrow.visible=false
	CardPool = newPool
	var max_pg = (CardPool.size()-1)/40
	...
	while i<40:
		if(1+i+page>CardPool.size()):
			...
			$Tw.interpolate_property(card_Array[i],"modulate", Color(0,0,0,-0.10*i), Color(1,1,1,1), 0.1+0.05*i, Tween.TRANS_SINE, Tween.EASE_IN)
			$Tw.start()
		i+=1
